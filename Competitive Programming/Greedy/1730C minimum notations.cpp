...
void solve()
{
    string str;
    cin >> str;
    //! Wrong approach:fails for 3 2 4 5 4
    // string sorted = str;
    // sortall(sorted);
    // string ans;
    // int i = 0, j = 0;
    // string deletion = "";
    // while (i < str.size() && j < sorted.size())
    // {
    //     if (str[i] == sorted[j])
    //     {
    //         ans.pb(str[i]);
    ...
    //         string temp = "";
    //         int k = i;
    //         while (i < str.size() && str[i] != sorted[j])
    //         {
    //             int ch = str[i] - '0';
    //             ch = min(ch + 1, 9);
    //             temp.pb(char('0' + ch));
...
/* 
* Go in reverse direction i.e from n - 2 to 0 WHY?
% because let's say we have a situation like this 72 if I delete 7 i'll get 28 if I delete 2 i'll get 37 as smallest.
% It is always better to delete elements from left side and insert them at right side
& Here is why Priority_queue is used, because we need to maintain the order of the elements in lexicographically smallest order.

? WE can avoid using pq by simply storing the smallest character occurred in the string while going through right to left
? if this smallest character so far obtained is greater than the current character at ith position then simply update the smallest character so far obtained
? else just simply overrite a[i] with min(a[i] + 1, 9) as per the question
? at the end simply sort it 
