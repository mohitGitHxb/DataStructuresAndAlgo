...
Egor has a deck of n
cards, the i-th card from the top has a number ai written on it. Egor wants to play a certain number of rounds until the cards run out. In each round, he takes a non-zero number of cards from the top of the deck and finishes the round. If the sum of the numbers on the cards collected during the round is between l and r
...
Egor knows by heart the order of the cards. Help Egor determine the maximum number of rounds he can win in such a game. Note that Egor is not required to win rounds consecutively.
 ...
 & always pick as minimum element possible so that the sum just exceeds or equals l
    & If sum > r start removing from start pointer [previous elements]
    & This is basically a sliding window problem
    & once we get a valid window put both pointers forward
 */
...
void solve()
{
    ...
    while (end < n)
    {
        ...
        {
            sum -= a[start];
            start++; // removing elements from start
        }
        if (sum >= l and sum <= r)
        {
            ans++;
            start = end + 1; // putting both pointers forward as the current window now has been used
            sum = 0;
        }
        end++;
    }
