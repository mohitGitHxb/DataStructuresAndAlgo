...
/*
The next lecture in a high school requires two topics to be discussed. The i-th topic is interesting by ai units for the teacher and by bi

units for the students.

The pair of topics i
and j (i<j) is called good if ai+aj>bi+bj
...
Your task is to find the number of good pairs of topics.
    ...
    @ Observations
    & First we need to isolate i and j terms (bring all to left side)
    & (ai - bi) + (aj - bj) > 0 OR we can say X + Y > 0
    % Now the question comes down to finding X and Y such that X + Y > 0 given i < j still
    % Since sum has to be greater than zero it can be seen that one of the element has to be non zero positive number (assume Y)
    % let's create a difference array which stores (ai - bi) for all i : 0 to n-1
    & Then we can see if X + Y > 0 then Y > -X or the minimum value of X such that X + Y > 0 would be min(X) = -Y + 1
    % Now we can solve this using binary search
    % Just find the index of Y let it be i and index of X such that X: -Y + 1 (say smaller)
    & The elements in the range [smaller,i] will make pair X + Y > 0 just keep a count of its occurence
...
void BruteForce()
{
    ...
    {
        int toFind = a[j] - b[j];
        auto elementsSmallerThanToFind = mp.lower_bound(toFind);
        for (auto it = mp.begin(); it != elementsSmallerThanToFind; it++)
        {
            if (it->F < toFind)
            {
                ans += it->S;
...
void solve()
{
    ...
    forn(i, n) cin >> b[i];
    vl difference(n);
    forn(i, n) difference[i] = a[i] - b[i];
    sortall(difference);
    ll ans = 0;
    customPrint(difference);
    forn(i,n){
        if(difference[i] <= 0) continue;
        int toFind = -difference[i] + 1;
        deb(toFind);
        int smaller = lower_bound(all(difference),toFind) - difference.begin();
        deb(smaller);
        ans += i - smaller;
    }
    cout << ans << "\n";
