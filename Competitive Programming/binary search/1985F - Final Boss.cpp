...
health. Your character has n attacks. The i'th attack deals ai damage to the boss but has a cooldown of ci turns, meaning the next time you can use this attack is turn x+ci if your current turn is x
 ...
 */
bool canBeat(mpll &mp, ll mid, ll h)
{
    unsigned long long totalDamage = 0;
    for (auto &i : mp)
    {
        ll cnt = (mid - 1) / i.F + 1; //& Important formula to get to know about how many times we'll be using this attack if given turns
        if (cnt >= h || totalDamage >= h)
        {
            return true;
        }
        totalDamage += i.S * cnt;
...
void solve()
{
    ...
    mpll mp; // stores <CoolDown, Damage>
    forn(i, n)
    {
        mp[c[i]] += a[i];
    }
    /*
    We can do this by binary searching for the answer. For some time t, we know that we can perform an attack of cooldown c exactly ⌊t−1c⌋+1 times. The total damage we will do in time t

will be:
∑i=1nai(⌊t−1ci⌋+1)

So we binary search for the first t
such that the total damage we do in time t is greater than or equal to h. This runs in O(nlog(h⋅maxci)). Be careful of long long overflow!
