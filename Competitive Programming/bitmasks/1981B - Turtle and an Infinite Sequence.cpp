...
will change to ai−1∣ai∣ai+1 for every positive integer i. a0 will change to a0∣a1. Here, |

denotes bitwise OR.

Turtle is asked to find the value of an
after m seconds. In particular, if m=0, then he needs to find the initial value of an. He is tired of calculating so many values, so please help him!
 */
...
void solve()
{
    ...
    cin >> n >> m;
    ll OR = 0;
    // forsn(i, max(n - m, 0), n + m + 1)
    // {
    //     OR |= i;
    // } // (n-m) to (n+m) this will throw TLE

    ll lower = max(n - m, 0);
    ll upper = n + m;

    //* O(log(n+m))
    // & Compute OR in log(n+m) time
    /*
    % first bit follows this pattern 010101010101
    % second bit follows this pattern 001100110011
    % third bit follows this pattern 000011110000
    % fourth bit follows this pattern 000000001111

    ^ we observed that ith bit will have 2^i 0 then 2^i ones
    ^ so to compute or effeciently we need to check any one bit in the ith bit column is set to 1
    % now we can divide this pattern 0000111100001111 into blocks 0000 1st block then next set of 0000 second block
    % block size = 2^i
    & Now to have a bit set in the entire column L and R should belong to different blocks (e.g : L:0000 11 R:11 0000 1111 )
