...
There is an array a
of length n, and you want to divide it into k>1 subsegments† in such a way that the MEX‡
...
†
A division of an array into k subsegments is defined as k pairs of integers (l1,r1),(l2,r2),…,(lk,rk) such that li≤ri and for each 1≤j≤k−1, lj+1=rj+1, and also l1=1 and rk=n
...
of the array [2,2,1] is 0, because 0
does not belong to the array.
MEX
of the array [3,1,0,1] is 2, because 0 and 1 belong to the array, but 2
does not.
MEX
of the array [0,3,1,2] is 4, because 0, 1, 2, and 3 belong to the array, but 4
...
void solve()
{
    ...
    vi a(n);
    forn(i, n) cin >> a[i];
    vi prefixMex(n), suffixMex(n);
    /*
    @ Important Observations
    -> if MEX of a subarray is x and another subarray MEX is also x then we can collectively combine these two subarrays to get
    & MEX(combined/merged subarrays) = x <-- Key observation
    ^ MEX of an array would decrease at all if we keep adding elements to the array
    % Now the problem boils down to simply find a valid split point in the array at which we can divide the array into TWO segments only such that their MEX are same

    % For this to be computed in O(n) we can use prefix and suffix MEX
     */

    //& computation of MEXs [important]
    hsi prefMex, suffMex;
    int mex = 0;
    forn(i, n)
    {
        prefMex.insert(a[i]);
        while (prefMex.count(mex))
        {
            mex++;
        }
        prefixMex[i] = mex;
    ...
    forn(i, n - 1)
    {
        ...
        {
            ans.eb(1, i + 1); // one based indexing [CODEFORCES FANTASY]
            ans.eb(i + 2, n);
            break; // ? we have found a valid split point
        }
    }
    if (ans.empty())
    {
