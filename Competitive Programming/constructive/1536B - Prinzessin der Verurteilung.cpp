...
or O(n) depending on how you implement substring checking.

 */
// custom comparator on basis of length then character for set<string>
struct Compare
{
...
set<string, Compare> allSubstrings;
...
void solve()
{
    ...
    string s;
    cin >> s;
    forv(it, allSubstrings)
    {
        bool flag = true;
        forn(i, n - sz(it) + 1)
        {
            // deb(s.substr(i, sz(it)));
...
int main()
{
    ...
    for (char i = 'a'; i <= 'z'; i++)
    {
        ...
        for (char j = 'a'; j <= 'z'; j++)
        {
            allSubstrings.insert(str + j);
            for (char k = 'a'; k <= 'z'; k++)
            {
                allSubstrings.insert(str + j + k);
            }
        }
    }
    // forv(i, allSubstrings){
    //     cerr << i << " ";
    // }
    // deb(allSubstrings.size());
    forn(i, t)
    {
