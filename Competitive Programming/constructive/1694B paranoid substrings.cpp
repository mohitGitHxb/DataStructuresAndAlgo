...
Let's call a binary string T of length m indexed from 1 to m paranoid if we can obtain a string of length 1 by performing the following two kinds of operations m−1

times in any order :

    Select any substring of T
...
For example, if T=
001, we can select the substring [T2T3] and perform the first operation. So we obtain T=

    01.

You are given a binary string S
of length n indexed from 1 to n. Find the number of pairs of integers (l,r) 1≤l≤r≤n such that S[l…r] (the substring of S from l to r) is a paranoid string.
...
void solve()
{
    ...
    ll ans = 1;
    forsn(i,1,n){
        // any combination of 11 or 00 will not form a paranoid string 
        if(s[i] == s[i-1]){
            ans++;
        }
        // 10 or 01 will form a paranoid string and using simple maths like finding the strings ending at current index (similar to sliding window problems) we can find the number of strings ending at current index
        else{
            ans+=(i+1);
        }
    }
