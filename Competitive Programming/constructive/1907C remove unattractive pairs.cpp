...
/*Vlad found a string s consisting of n
...
void solve()
{
    ...
    & if any character is appearing for more than n/2 times, then even if we try to remove all pairs optimally,
    & we would still remain with the same character in the string (whole string  consisting of same characters)
    & characters that will stay unaffected will be 2 * maximum occurrence - size of string this can be observed easily
    % else we can always remove all characters from a string if string size is of even length otherwise one element will be left
     */
