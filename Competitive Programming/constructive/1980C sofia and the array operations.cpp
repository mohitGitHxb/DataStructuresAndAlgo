...
void solve()
{
    ...
    /*
    & Last modification has to be present in the modified array
    & now for rest of the a[i] != b[i] the corresponding b[i] has to be present in the dmap

    & all other entries in dmap which are not equal to any b[i] must be over written by the last element (step - 1)
     */
    if (count(all(b), d.back()) == 0)
    {
        puts("NO");
        return;
    }
