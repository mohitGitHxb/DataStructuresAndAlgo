...
Each branch is an isosceles triangle with base d
and height h, whose base is perpendicular to the trunk. The triangles are arranged upward at an angle, and the trunk passes exactly in the middle. The base of the i-th triangle is located at a height of yi

.

The figure below shows an example of a tree with d=4,h=2
and three branches with bases at heights [1,4,5].

 */

/*

@ Must know the concept of similarity of triangles
Let's consider the triangles in ascending order of yi. Let the current triangle have index i

. There are two cases:

    The triangle does not intersect with the (i+1)

-th triangle (yi+1−yi≥h). In this case, we simply add the area of the triangle to the answer. The area will be d⋅h2
.
The triangle intersects with the (i+1)
-th triangle (yi+1−yi<h). We can add to the answer the area of the figure that does not belong to the intersection and move on to the next triangle. Note that this figure is a trapezoid with a lower base d and height h′=yi+1−yi. The upper base can be found based on the similarity of triangles. The heights of the triangles are in the ratio k=h−h′h. Then the upper base dtop=d⋅k. The area of the trapezoid is h′⋅d+dtop2.
...
void solve()
{
    ...
    sortall(a);
    double answer = areaOfTriangle(d, h);
    forn(i, n - 1)
    {
        if (a[i + 1] - a[i] < h)
        {
            int gap = (a[i + 1] - a[i]);
            double base = d * ((h - gap) * 1.0 / h * 1.0);
            answer += areaOfTrapezium(d, base, gap);
        }
        else
        {
            answer += areaOfTriangle(d, h);
        }
    }
