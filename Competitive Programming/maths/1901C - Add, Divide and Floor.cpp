...
/*
You are given an integer array a1,a2,…,an (0≤ai≤109). In one operation, you can choose an integer x (0≤x≤1018) and replace ai with ⌊ai+x2⌋ (⌊y⌋ denotes rounding y down to the nearest integer) for all i from 1 to n
...
over all testcases doesn't exceed 2⋅105.
 ...
 @ Observations
 &  Order doesn't matter and if minElement == maxElement that means every element is same
 & Performing the operation (a_i + x) / 2 doesn't affect the sorted order hence we can just try to make minElement == maxElement
 & We can either pick MaximumElement and perform operations according to that or pick minElement and perform elements according to that

 % Pick minimum check if it is odd or even why? (a_i + x) has to be even
 % in order to nullify the effect of the floor operation
 */
...
void solve()
{
    ...
    {
        // if minElement is even that means we can choose X = 0 so that the floor operations becomes nullified
        if (minElement % 2 == 0)
        {
            minElement /= 2;
            maxElement /= 2;
        ...
        // if minElement is odd that means we can choose X = 1 so that the floor operations becomes nullified
        else
        {
            minElement = (minElement + 1) / 2;
