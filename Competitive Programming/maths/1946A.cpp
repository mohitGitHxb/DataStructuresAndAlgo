...
/*
Read input test cases.Sort the array.Calculate the index of the current median.Iterate from the current median to the end of the array.For each element after the median, if it's less than or equal to the current median, increment it to make it greater than the current median.Count the number of operations required.Output the result.
 */
...
void solve()
{
    ...
    sortall(a);
    int idx = upper_bound(all(a), a[(n + 1) / 2]) - a.begin();
    idx--;
    int ans = 0;
    cout << idx - (n + 1) / 2 + 1 << "\n";
}
