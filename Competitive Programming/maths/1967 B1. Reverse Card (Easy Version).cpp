...
Calculate the number of ordered pairs (a,b)
...
void solve()
{
    ...
    ll ans = 0;
    /* 
    & As per queston (a + b) % b*gcd(a,b) has to be 0;
    & From testcases it can be deduced that if gcd(a,b) == b then there is a pair formation
    & because if gcd(a,b) == b that means a is divisible by b and it also means (a+b) is divisible by b as well;

    ~ So if we replace gcd(a,b) with b we get (a+b) % b*b == 0
    ~ but how can you find the count of elements divisible by b in a range

    % For example if we have [1...10] and you want to find count of elements divisible by 3 then ans would be floor of 10/3 = 3
    ~ Now apply a little manipulation here instead of (a+b)/b^2 lets separate two b in denominator

    & (a+b)/b^2 --> a/b^2 + 1/b --> (a/b + 1) / b <-- this gives the count of divisible elements by b 
    
     */
    for(ll b = 1; b <= m; b++)
    {
        ll x = n / b + 1;
        ll y = x / b; // this statement is equivalent to (a + b) / b^2 
        ans += y;
    }
    ans--; // ? Needed because the answers were coming +1, 
    cout << ans << "\n";
}
