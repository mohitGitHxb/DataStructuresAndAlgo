...
void solve()
{
    ...
    map<tuple<int, int, int>, int> exactMatch;
    mp.insert({{-1e9, -1e9, -1e9}, 1});
    /*

    ^ Quick Feedback:
    take (a1,a2,a3) and now you have 3 possibilities to get valid pair.
    ! useBrain();



    & For a triplet [a1,a2,a3] we will check whether the following exists or not
    % [a1,a2,-1e9], [a1,-1e9,a2], [-1e9,a1,a2]
    & And for each triplet we will also store all 3 combos like that
    & However there is a chance that same triplet is present multiple times so we need to subtract it from the answer
     */
    ll ans = 0;
    forn(i, n - 2)
    {
        tuple<int, int, int> s = {a[i], a[i + 1], a[i + 2]};
        tuple<int, int, int> t = {a[i], a[i + 1], -1e9};
        if (mp.count(t))
        {
            ans += mp[t];
            ans -= exactMatch[s];
        }
        tuple<int, int, int> u = {a[i], -1e9, a[i + 2]};
        if (mp.count(u))
        {
            ans += mp[u];
            ans -= exactMatch[s];
        }
        tuple<int, int, int> v = {-1e9, a[i + 1], a[i + 2]};
        if (mp.count(v))
        {
            ans += mp[v];
            ans -= exactMatch[s];
