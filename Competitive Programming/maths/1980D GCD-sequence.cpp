...
void solve()
{
    ...
    /* 
    Let's loop through the initial array a, counting the GCD of neighbouring elements. If at some point, the GCD of the previous pair becomes greater than the GCD of the next pair, we remember the index i

of the first element of the pair that gave the greater GCD and stop the loop.

Then consider the following cases:

    the sequence of GCDs for array a

was already non-decreasing. Then it is enough to remove the last element from it, the previous elements of the GCD-sequence will not be affected. The answer in this case is always YES.
some element ai
has been found such that GCD(ai−1,ai)>GCD(ai,ai+1). Then, since there is already at least one place in the original GCD-sequence where it ceases to be non-decreasing, we can try to fix it by removing the i−1-th, i-th or i+1-th element from the array a. Let's create 3 new arrays of length n−1, and in each of them remove one of the above elements. If at least one of these arrays has a non-decreasing GCD-sequence — the answer is YES, otherwise — NO. 
    
    & Pretty much brute forcing all 3 possibilities that's it!
    ...
    ? Remove element at dipIndex - 1, dipIndex, dipIndex + 1 that's it
    
     */
    forsn(ignoreIdx, dipIndex - 1, dipIndex + 2)
    {
        vl c;
        vl d = a;
        // cerr << "Element to be removed : " << d[ignoreIdx] << "\n";
        d.erase(d.begin() + ignoreIdx);
        forsn(i, 1, sz(d))
        {
            c.pb(__gcd(d[i - 1], d[i]));
        }
        // customPrint(c);
        if (is_sorted(all(c)))
        {
