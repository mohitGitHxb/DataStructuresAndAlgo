...
void solve()
{
    ...
    {
        /* 
        & The below code ensures that we don't end up getting TLE!
        & first of all imagine case x = 3,y = 10,k = 1e5 
        & now you need y - (x % y) steps to make x % y == 0 
        & and once it does it becomes 0 then after it it'll become x = 1;

        % Now comes the interesting part if x = 0, y = 10 we require +10 steps
        % because everytime we first do x++ and then check if x % y == 0
        % now the cycle of x = 0,y = 10 repeats k / (y - x) times easily
        % once k < (y-x) then only x will increase upto x + k
