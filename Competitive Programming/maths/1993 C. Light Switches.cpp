...
minutes — it turns on the light for k minutes, then turns it off for the next k minutes, then turns it back on for the next k minutes, and so on. In other words, the light status is changed by the chip at minute ai, ai+k, ai+2k, ai+3k, … for the i

-th room.

What is the earliest moment when all rooms in the apartment have their lights turned on?

    % First observation : [ai to ai + k - 1] lights are on then [ai + 2k, ai + 3k - 1] lights are on and so on.
    & Generalized form ai,ai+2k,ai+4k,ai+nk lights are on where n is a even number;
    & how to make all the lights on at the same time?
        ~ the answer would be in the range max(a) to max(a) + k - 1 because all other ai are smaller than this
    @ Brute force: TLE
    &- Iterate over all values from max(a) to max(a) + k - 1
    &- check if all the lights are on at the same time or not

    @ Optimal solution :
    &- Draw the intervals and you will notice the overlapping region for all ai
    % If max(starting point of interval) <= min(ending point of interval) then answer exists and it would be max(starting point of interval)
    % Else answer does not exist
 */

bool isLightOn(int a, int k, int currentTime)
{
    int n = (currentTime - a) / k;
    return (n % 2 == 0); // as per the observation this has to be even number
}
...
void solve()
{
    ...
    vl a(n);
    forn(i, n) cin >> a[i];
    // sortall(a);  order doesn't matter hence sort it [you don't have to as the only concern is that the answer is in the range max(a) to max(a) + k - 1]
    ll maxElement = MAX(a);
    //~ Making all elements as close to maxElement as possible [to get interval overlapping]
    forn(i, n)
    {
        ll factor = (maxElement - a[i]) / k;
        if (factor & 1)
        {
            factor++;
        }
        //? again remember that factor has to be even for light to be turned on
        a[i] += factor * k;
    }
    ll maxLeft = maxElement, minRight = maxElement + k - 1;
    forn(i, n)
    {
        maxLeft = max(maxLeft, a[i]);
        minRight = min(minRight, a[i] + k - 1);
    }
    if (maxLeft <= minRight)
    {
        cout << maxLeft << "\n";
