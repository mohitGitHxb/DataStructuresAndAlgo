...
/*
As the CEO of a startup company, you want to reward each of your k employees with a ticket to the upcoming concert. The tickets will be on sale for n days, and by some time travelling, you have predicted that the price per ticket at day i will be ai

. However, to prevent ticket hoarding, the concert organizers have implemented the following measures:

    A person may purchase no more than m

tickets per day.
If a person purchases x
tickets on day i, all subsequent days (i.e. from day i+1 onwards) will have their prices per ticket increased by x

    .

For example, if a=[1,3,8,4,5]
and you purchase 2 tickets on day 1, they will cost 2 in total, and the prices from day 2 onwards will become [5,10,6,7]. If you then purchase 3 more tickets on day 2, they will cost in total an additional 15, and the prices from day 3 onwards will become [13,9,10]

.

Find the minimum spending to purchase k

tickets.


@ observations:
&- Break the actual cost into = constant cost + inflated cost (due to purchase of tickets)
&- It is always better to purchase more tickets at the same day to avoid getting inflation in further rounds
&- Order of tickets doesn't matter
 */
...
void solve()
{
    ll n, m, k;
    cin >> n >> m >> k;
    multiset<ll> sortedTickets;
    forn(i, n)
    {
        ll x;
        cin >> x;
        sortedTickets.insert(x);
    }
    ll constantCost = 0; // constant cost 
    ll boughtTickets = 0; // this is equal to inflationCost
    forv(ticket, sortedTickets)
    {
        ll minimum = min(k - boughtTickets, m);
        ll cost = minimum * (ticket + boughtTickets);
        constantCost += cost;
        boughtTickets += minimum;
    }
    cout << constantCost << "\n";
}
