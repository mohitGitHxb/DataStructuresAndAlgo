...
Given n strings, each of length 2, consisting of lowercase Latin alphabet letters from 'a' to 'k', output the number of pairs of indices (i,j) such that i<j and the i-th string and the j

-th string differ in exactly one position.

In other words, count the number of pairs (i,j)
(i<j) such that the i-th string and the j-th string have exactly one position p (1≤p≤2) such that sip≠sjp
...
void solve()
{
    ...
    forv(str, s)
    {
        //& Brute force all possible pairs that can either be formed using first letter or 2nd letter
        //& but we will end up counting the pairs twice so make sure to decrease their count also
        ans += (upperRow[str[0] - 'a'] + lowerRow[str[1] - 'a'] - 2 * mpx[str]);
        upperRow[str[0] - 'a']++;
        lowerRow[str[1] - 'a']++;
        mpx[str]++;
    }
