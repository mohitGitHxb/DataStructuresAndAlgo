...
public:
    ...
    {
        vvi suffixBottomMaximums(sz(grid), vi(sz(grid[0]), -1e9));
        vvi suffixRightMaximums(sz(grid), vi(sz(grid[0]), -1e9));
        forn(col, sz(grid[0]))
        {
            rforn(row, sz(grid) - 2)
            {
                suffixBottomMaximums[row][col] = max(suffixBottomMaximums[row + 1][col], grid[row + 1][col]);
            }
        }
        // customPrint(suffixBottomMaximums);
        forn(row, sz(grid))
        {
            rforn(col, sz(grid[0]) - 2)
            {
                suffixRightMaximums[row][col] = max(suffixRightMaximums[row][col + 1], grid[row][col + 1]);
            }
        }
        // customPrint(suffixRightMaximums);
        vvi suffixCombinedMaximums(sz(grid), vi(sz(grid[0]), -1e9));
        suffixCombinedMaximums.back() = suffixRightMaximums.back();
        forn(row, sz(grid))
        {
            suffixCombinedMaximums[row][sz(grid[0]) - 1] = suffixBottomMaximums[row][sz(grid[0]) - 1];
        }
        rforn(row, sz(grid) - 2)
        {
            rforn(col, sz(grid[0]) - 2)
            {
                suffixCombinedMaximums[row][col] = max({suffixCombinedMaximums[row + 1][col], suffixCombinedMaximums[row][col + 1], suffixCombinedMaximums[row + 1][col + 1], suffixBottomMaximums[row][col], suffixRightMaximums[row][col]});
            }
        }
        // customPrint(suffixCombinedMaximums);
        int ans = INT_MIN;
        forn(row, sz(grid))
        {
            forn(col, sz(grid[0]))
            {
                if (row == sz(grid) - 1 && col == sz(grid[0]) - 1)
                    continue;
                ans = max(ans, -grid[row][col] + suffixCombinedMaximums[row][col]);
                // if (col + 1 < sz(grid[0]) && row + 1 < sz(grid))
                // {
                //     ans = max(ans, grid[row][col] - max(suffixBottomMaximums[row][col + 1], suffixRightMaximums[row + 1][col]));
                // }
            }
        }
        return ans;
