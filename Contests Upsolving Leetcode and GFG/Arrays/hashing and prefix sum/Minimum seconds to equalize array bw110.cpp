...
! Wrong intuition: taking the highest frequent element the computing the ans by using its indexes
! This will fail because let's say if all highest frequent elements are consecutively placed then the extreme ends will be so far
! there is a high chance element with let's say more even distribution will give lower answer

& Try for all elements and calculate the answer 
& between 5 to 8 there are two indexes 6,7 they will require atleast 2 sec
& using above information for even distance and odd distance just find out the total seconds 
& make sure to minimize it as well

* Time complexity : O(Unique_elements * n)
