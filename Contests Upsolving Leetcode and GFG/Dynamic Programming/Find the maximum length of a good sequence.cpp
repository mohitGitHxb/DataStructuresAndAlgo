...
public:
    ...
    int dp[501][501][26];
    int solve(vi &nums, int idx, int prev, int k)
    {
        if (idx == nums.size())
        {
            return 0;
        ...
        int notTake = solve(nums, idx + 1, prev, k);
        int take = -1e9;
        if (prev == -1 || nums[prev] == nums[idx])
        {
            take = 1 + solve(nums, idx + 1, idx, k);
        }
        else if (k > 0)
        {
            take = 1 + solve(nums, idx + 1, idx, k - 1);
        }
    ...
    int maximumLength(vector<int> &nums, int k)
    {
        ...
        forn(i, sz(nums))
        {
            rforn(remainderK, k)
            {
                dp[nums[i]][remainderK] = max(dp[nums[i]][remainderK] + 1, (remainderK > 0 ? 1 + maxDPforK[remainderK - 1] : 0));

                maxDPforK[remainderK] = max(maxDPforK[remainderK], dp[nums[i]][remainderK]);

                ans = max(ans, dp[nums[i]][remainderK]);
            }
        }
