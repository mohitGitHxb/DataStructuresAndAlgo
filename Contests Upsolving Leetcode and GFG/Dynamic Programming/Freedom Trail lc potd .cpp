...
public:
    ...
    vvi dp;
    //* T.C O(N*M*N)
    int findRotateSteps(int ringIndex, int i, int ringLength)
    {
        int clockWiseDistance = abs(ringIndex - i);
    ...
    int solve(int ringIndex, int keyIndex, string &ring, string &key)
    {
        ...
        forn(i, sz(ring))
        {
            // checking if the ith element in the ring is equal to the keyIndex'th element in the key given that assuming ringIndex as starting point
            if (ring[i] == key[keyIndex])
            {
                //? get the minimum distance to reach the required character key[keyIndex] then add 1 (pressing button cost) and recursively call for the next keyIndex
                int totalSteps = findRotateSteps(ringIndex, i, sz(ring)) + 1 + solve(i, keyIndex + 1, ring, key);
                minimumSteps = min(minimumSteps, totalSteps);
            }
        }
        return dp[ringIndex][keyIndex] = minimumSteps;
    }
    ...
    int bottomUp(string ring, string key)
    {
        int n = sz(ring), m = sz(key);
        vvi table(n, vi(m + 1, 0));
        rforn(i, m - 1)
        {
            rforn(j, n - 1)
            {
                ...
                forn(k, sz(ring))
                {
                    if (ring[k] == key[i])
                    {
                        int totalSteps = findRotateSteps(j, k, sz(ring)) + 1 + table[k][i + 1];
                        minimumSteps = min(minimumSteps, totalSteps);
                    }
                }
                table[j][i] = minimumSteps;
            }
        }
        return table[0][0];
    }
    int spaceOptimized(string ring, string key)
    {
        int n = sz(ring), m = sz(key);
        ...
        rforn(i, m - 1)
        {
            rforn(j, n - 1)
            {
                ...
                forn(k, sz(ring))
                {
                    ...
                    {
                        //! checkout this step
                        int totalSteps = findRotateSteps(j, k, sz(ring)) + 1 + ahead[k];
                        minimumSteps = min(minimumSteps, totalSteps);
                    }
                }
                curr[j] = minimumSteps;
