...
Explanation: One solution is to remove the elements at indices 0, 1, and 5, making the array nums = [1,5,6,3,1].
     ...
     */
public:
    vi longestSubsequence(vi &nums, bool reverse = false)
    {
        vi dp(sz(nums), 1);
        if (!reverse)
        {
    ...
    int minimumMountainRemovals(vi &nums)
    {
        vi longestIncreasingSubsequence(longestSubsequence(nums));
        vi longestDecreasingSubsequence(longestSubsequence(nums, true));
        int maxLength = 1;
        forsn(i, 0, sz(nums))
        {
            if (longestIncreasingSubsequence[i] > 1 && longestDecreasingSubsequence[i] > 1) // because mountain must have atleast length 3
                maxLength = max(maxLength, longestIncreasingSubsequence[i] + longestDecreasingSubsequence[i] - 1);
        }
        return sz(nums) - maxLength;
    }
};
