...
! Messed up in contest because of not thinking of iterating only on unique values
& Cannot iterate on power array as it is very complex to find which spell to take and which ones to ignore
& to simplify the process of choosing spells we can sort the array (it is not going to change our answer)
% however for ex 1,1,3,4,5,5,5,6,7,8,8 here if we are at element 4 we can't take 5,5,5,6 because according to problem statement we can choose X+1,X+2
% Note: X-1,X-2 are already taken care since the array is sorted
? To fix the issue we can have a vector of pair like this <power,totalDamage>
& Doing this guarantees we only have unique values for ex: <1.2>,<3,3>,<4,4>,<5,15>,<6,6>,<7,7>,<8,16>

& Now simply using take nottake dp we can solve this problem remember 
& at index = idx we only need to check first element which is allowed
^ check for these only idx+1,idx+2,idx+3 that's it!
...
public:
    ...
    vl dp;
    ll solve(vpl &uniquePowers, int idx)
    {
        if (idx == uniquePowers.size())
        {
            return 0;
        }
        if (dp[idx] != -1)
        ...
        ll notTake = solve(uniquePowers, idx + 1);
        ll maxTake = uniquePowers[idx].second;
        for (int i = idx + 1; i <= min((int)uniquePowers.size() - 1, idx + 3); i++)
        {
            if ((uniquePowers[i].first - uniquePowers[idx].first) > 2)
            {
                maxTake = max(maxTake, uniquePowers[idx].second + solve(uniquePowers, i));
                break;
            }
        }
        return dp[idx] = max(notTake, maxTake);
    }
    ll maximumTotalDamage(vi &power)
    {
        mpll damageDealt;
        for (int &i : power)
        {
        ...
        vpl uniquePowers;
        for (auto &i : damageDealt)
        {
            uniquePowers.eb(i.first, i.second);
        }
        dp.resize(sz(uniquePowers) + 1, -1);
        return solve(uniquePowers, 0);
    }
};
