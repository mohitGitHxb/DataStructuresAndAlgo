...
public:
    ...
    int solve(string &s, int idx)
    {
        ...
        /*
        @ Important Note
        HINT: a string is balanced iff max(freq)*unique == n
HINT 2: you can accumulate both max freq and unique in O(1) time as you're iterating

Now you can check if the string is balanced in O(1)
        ...
        for (int i = idx; i < s.size(); i++)
        {
            ...
            if (maxFreq * mp.size() == i - idx + 1)
            {
                int temp = 1 + solve(s, i + 1);
                ans = min(ans, temp);
            }
        }
        return ans;
