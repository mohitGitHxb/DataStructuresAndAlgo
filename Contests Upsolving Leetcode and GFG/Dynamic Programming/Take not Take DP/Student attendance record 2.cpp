...
public:
    ...
    ll dp[100001][3][2];
    ll solve(int idx, int streakOfL, bool cntA, int &n)
    {
        if (idx == n)
        {
            return 1;
        }
        if (dp[idx][streakOfL][cntA] != -1)
        {
            return dp[idx][streakOfL][cntA];
        }
        ll choice1{0}, choice2{0}, choice3{0}; // & 3 choices we have heres
        choice1 = solve(idx + 1, 0, cntA, n);
        if (!cntA) //& in whole string there is only single absent possible
        {
            choice2 = solve(idx + 1, 0, true, n);
        }
        if (streakOfL + 1 < 3) //& in whole string there is atmost 2 consecutive late possible
        {
            choice3 = solve(idx + 1, streakOfL + 1, cntA, n);
        }
        return dp[idx][streakOfL][cntA] = (choice1 + choice2 + choice3) % mod;
    }
