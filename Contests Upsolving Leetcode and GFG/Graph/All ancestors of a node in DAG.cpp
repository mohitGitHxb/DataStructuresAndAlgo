...
public:
    ...
    vvi getAncestors(int n, vvi &edges)
    {
        vvi ans(n, vi());
        vi indegree(n);
        vvi adj(n);
        ...
        // Applying Kahn's algorithm for topological sort
        // whichever has lowest degree we'll traverse from it
        queue<int> q;
        forn(i, n)
        {
            if (indegree[i] == 0)
        ...
        customPrint(topoSort);
        unordered_map<int, si, custom_hash> mp;
        ...
        while (!topoSort.empty())
        {
            ...
            forv(child, adj[node])
            {
                mp[child].insert(node);
                // it has previous anchestors
                if (!mp[node].empty())
                {
                    mp[child].insert(mp[node].begin(), mp[node].end());
                }
            }
        }
        forn(i, n)
        {
            if (mp.count(i))
            {
                ans[i].assign(mp[i].begin(), mp[i].end());
            }
        }
        return ans;
    }
