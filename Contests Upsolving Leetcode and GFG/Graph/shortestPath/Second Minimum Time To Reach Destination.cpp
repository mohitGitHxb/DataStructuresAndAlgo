...
Notes:
    ...
    @ Feedback:
    ! wrong appraoch: applying dijkstras and then at the last node going one step back and one step forward
    &- Correct approach
...
public:
    ...
    int calculateDelay(int currTime, int change)
    {
        int extra = change - currTime % change;
    ...
    int dijkstra(vector<vpii> &graph, int source, int &time, int &change)
    {
        int n = graph.size();
        ...
        while (!pq.empty())
        {
            ...
            for (auto &edge : graph[node])
            {
                ...
                int finalDis = dis + nextDis + calculateDelay(dis, change);

                if (finalDis < dist1[nextNode])
                {
                    dist2[nextNode] = dist1[nextNode];
                    dist1[nextNode] = finalDis;
                    pq.push({finalDis, nextNode});
                }
                else if (finalDis > dist1[nextNode] && finalDis < dist2[nextNode])
                {
                    dist2[nextNode] = finalDis;
                    pq.push({finalDis, nextNode});
                }
            }
        }
    ...
    int secondMinimum(int n, vvi &edges, int time, int change)
    {
        ...
        return dijkstra(graph, 1, time, change);
    }
};
