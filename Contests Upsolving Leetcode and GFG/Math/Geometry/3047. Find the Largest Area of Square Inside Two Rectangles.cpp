...
public:
    ...
    // & condition to check whether two rectangles are overlapping by comparing their bottom left and top right coordinates
    bool isOverlapping(vi &bl1, vi &tr1, vi &bl2, vi &tr2)
    {
        bool x = tr1[0] >= bl2[0] && tr2[0] >= bl1[0];
        bool y = tr1[1] >= bl2[1] && tr2[1] >= bl1[1];
        return x & y;
    }
    ll getArea(vi &bl1, vi &tr1, vi &bl2, vi &tr2)
    {
        ll minY = min(tr1[1], tr2[1]);
        ll maxY = max(bl1[1], bl2[1]);
        ll minX = min(tr1[0], tr2[0]);
        ll maxX = max(bl1[0], bl2[0]);
        ll side = min((minY - maxY), (minX - maxX));
        return side * side;
    }
    long long largestSquareArea(vector<vector<int>> &bottomLeft, vector<vector<int>> &topRight)
    {
        ll area = 0;
        forn(i, sz(bottomLeft))
        {
            forsn(j, i + 1, sz(topRight))
            {
                if (isOverlapping(bottomLeft[i], topRight[i], bottomLeft[j], topRight[j]))
                {
                    area = max(area, getArea(bottomLeft[i], topRight[i], bottomLeft[j], topRight[j]));
                }
            }
        }
        return area;
