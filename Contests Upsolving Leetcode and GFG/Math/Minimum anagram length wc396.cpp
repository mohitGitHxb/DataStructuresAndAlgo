...
public:
    ...
    bool isAnagram(vi &baseFreq, string &s, int divisor)
    {
        for (int i = divisor; i < s.length(); i += divisor)
        {
            vi freq(26);
    ...
    int minAnagramLength(string s)
    {
        ...
        for (int i = 1; i * i <= s.length(); i++)
        {
            if (s.length() % i == 0)
            {
                ...
                if (i != s.length() / i)
                {
                    divisors.pb(s.length() / i);
                }
            }
        }
        forv(divisor, divisors)
        {
            vi baseFreq(26);
            forn(i, divisor)
            {
            ...
            if (isAnagram(baseFreq, s, divisor))
            {
                return divisor;
            }
        }
