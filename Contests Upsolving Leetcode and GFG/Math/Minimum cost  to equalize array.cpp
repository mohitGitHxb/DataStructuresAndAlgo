...
! THIS QUESTION IS A NIGHTMARE Either you know the code or you fail to solve !!
     */
public:
    const int mod = 1e9 + 7;
    Solution()
    ...
    int minCostToEqualizeArray(vi &nums, int cost1, int cost2)
    {
        ...
        //* case 2
        ll op1 = max(0ll, (mx - mn) * 2 - totalGap);
        ll op2 = totalGap - op1;
        ll res = (op1 + op2 % 2) * cost1 + op2 / 2 * cost2;
        //* case 3
        totalGap += op1 / (n - 2) * n;
        op1 %= n - 2;
        op2 = totalGap - op1;
        ...
        {
            totalGap += n;
            res = min(res, totalGap % 2 * cost1 + totalGap / 2 * cost2);
        }
