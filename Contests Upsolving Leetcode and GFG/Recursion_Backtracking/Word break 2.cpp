...
public:
    ...
    vsr result;
    void helper(string &s, unordered_set<string> &dict, int index, string curr)
    {
        if (index == s.length())
        {
        ...
        string word = "";
        forsn(i, index, s.length())
        {
            word += s[i];
            if (dict.count(word))
            {
                helper(s, dict, i + 1, curr + word + " ");
            }
        }
    }
    vsr wordBreak(string s, vsr &wordDict)
    {
