...
public:
    ...
    /*
    @ KEY observations: [sliding window,binary search]
    ^ n is the size of the UNIQUENESS ARRAY which can be calculated as n*(n+1)/2 [total amount of subarrays of original array nums]
    % Median of an array is the middle element when the array is sorted i.e a[n/2];
    % that means atleast ceil(n/2) has to be less than the median

    & if we observe carefully we can notice that median of the uniqueness array will always lie in the range [1,total no. of distinct elements] <-- binary search on space hints

    & hence we can apply binary search on the range and the black box function (isValid) can find the number of subarrays that have atleast "mid" distinct elements in them
    & if this mid reaches ceil(n/2) we got our answer (make sure to always try to go left : high--)

    & To implement the black box function we will use a hashmap
    & Also we will use a variable count to count the number of subarrays that have atleast mid distinct elements in them
    ...
    *Time complexity : O(NlogN)
    ...
    bool isValid(vi &nums, int mid)
    {
        int start = 0, end = 0;
        ll sizeOfUniquenessArray = (sz(nums) * (sz(nums) + 1)) / 2;
        hmpi uniques;
        ll count = 0;
        while (end < sz(nums))
        {
            uniques[nums[end]]++;
            while (uniques.size() > mid)
            {
                uniques[nums[start]]--;
                if (uniques[nums[start]] == 0)
                {
                    uniques.erase(nums[start]);
                }
                start++;
            }
            count += end - start + 1; // count no. of subarrays that have atleast mid distinct elements
            end++;
        }
        return count >= (sizeOfUniquenessArray + 1) / 2;
    }
    int medianOfUniquenessArray(vi &nums)
    {
        int low = 1;
        int ans = 0;
        int high = unordered_set<int>(all(nums)).size();
        while (low <= high)
        {
            ...
            {
                ans = mid;
                high = mid - 1; // all values > mid will have atleast mid distinct elements so we can ignore those elements
            }
            else
            {
                low = mid + 1;
