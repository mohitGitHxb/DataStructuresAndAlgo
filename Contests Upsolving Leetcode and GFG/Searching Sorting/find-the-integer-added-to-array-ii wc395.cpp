...
public:
    ...
    bool isPossible(vi &nums1, vi &nums2, int diff)
    {
        int neglect = 2;
        for (int i = 0, j = 0; i < sz(nums1) and j < sz(nums2); i++)
        {
            ...
            {
                neglect--;
                if (neglect < 0 or diff > nums2[j] - nums1[i])
                    return false;
            }
        }
        return neglect >= 0;
    }
    int minimumAddedInteger(vi &nums1, vi &nums2)
    {
        sortall(nums1);
        sortall(nums2);
        int minDiff = INT_MAX;
        forn(i, sz(nums1))
        {
            int diff = nums2.front() - nums1[i];
            // check if this diff is possible to achieve
