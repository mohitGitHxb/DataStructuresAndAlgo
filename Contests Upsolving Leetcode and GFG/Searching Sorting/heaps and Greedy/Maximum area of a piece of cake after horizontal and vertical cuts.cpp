...
Output: 4
Explanation: The figure above represents the given rectangular cake. Red lines are the horizontal and vertical cuts. After you cut the cake, the green piece of cake has the maximum
...
public:
    ...
    {
        sortall(horizontalCuts);
        sortall(verticalCuts);
        int maximumHorizontalDistance = max(horizontalCuts[0], h - horizontalCuts.back());
        int maximumVerticalDistance = max(verticalCuts[0], w - verticalCuts.back());
        forsn(i, 1, sz(horizontalCuts))
        {
            maximumHorizontalDistance = max(maximumHorizontalDistance, horizontalCuts[i] - horizontalCuts[i - 1]);
        }
        forsn(i, 1, sz(verticalCuts))
        {
            maximumVerticalDistance = max(maximumVerticalDistance, verticalCuts[i] - verticalCuts[i - 1]);
        }
        return ((maximumHorizontalDistance % mod) * 1LL * (maximumVerticalDistance % mod)) % mod;
    }
};
