...
public:
    ...
    double mincostToHireWorkers(vi &quality, vi &wage, int k)
    {
        ...
        forn(i, sz(quality))
        {
            workers.pb({double(wage[i]) / quality[i], quality[i]}); //? we need to pick cheapest workers available
            //? so we need to sort them according to their ratio
        }
        sortall(workers);
        double ans = DBL_MAX;
        ...
        {
            /*
            % InsteaD of performing n^2 checks by picking up all the workers ratio and then comparing them with all remaining workers
            % We can just simply maintain a heap of size k and compare it with all the workers
            @ Price to be paid for ith worker = quality[i] * ratio of wage[i]/quality of that worker whom we are paying the least

            ? Instead of performing quality[i] * ratio for every K workers,
            it is better to just choose K workers having least quality and multiply it with ratio

            * Min(q1,q2,q3,...,qk) * ratio;

            * O(NlogN + NlogK) T.C

            */
            quantitySum += workers[i].second;
            ...
            if (pq.size() == k)
            {
                ans = min(ans, workers[i].F * quantitySum);
            }
        }
        return ans;
    }
