...
public:
    ...
    {
        int patches = 0;
        ll maximumReach = 0, numberToAdd = 0;
        int i = 0;
        while (maximumReach < n)
        {
            /*
            & Number isn't reachable if current maximum reach is less than current number by value of 1 means nums[i] > maximumReach + 1

            & Now we have no choice but to add number nums[i] to reach maximumReach + 1

            & What number should we add but?
            % The answer is simple if we are able to reach till X and want to reach further obviously element with value X + 1 is required
            % always pick smallest value to add as it will cover more range than larger number for ex we could've added X+2,X+3 but we may not be able to cover X+1
            % To avoid this always pick X + 1 X : maximumReach


             */
            if (i == nums.size() || nums[i] > maximumReach + 1)
            {
                numberToAdd = maximumReach + 1;
                maximumReach = numberToAdd + maximumReach;
                patches++;
            }
            else
            {
