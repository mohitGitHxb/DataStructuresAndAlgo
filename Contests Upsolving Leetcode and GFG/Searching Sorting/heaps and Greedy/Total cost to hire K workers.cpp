...
The total hiring cost is 4.
    ...
    @ Feedback / approach
    & Read the question carefully and PLEASE DRY RUN THE GOD DAMN TESTCASES!! Wasted 20mins just to realise this question wasn't that complex as we thought

    & Simple min heap based solution
    % get first [candidates] workers and last [candidates] workers
    % take out whichever is minimum and add it to ans and accordinly update heaps / multisets

    * TC - O( (k + candidates) * log(k * candidates))
     */
public:
    const int mod = 1e9 + 7;
    Solution()
    ...
    long long totalCost(vector<int> &costs, int k, int candidates)
    {
        ...
        {
            /* 
            % i <= j because we want to get first [candidates] workers and last [candidates] workers
            % j >= i because i would have gone one more step (it needed to include 4 elements but after running first while loop it'll be at next position 5)
            ^ just perform a dry run
            
             */
            while (i <= j && s1.size() < candidates)
            {
                s1.insert(costs[i]);
                i++;
            }
            while (j >= i && s2.size() < candidates)
            {
                s2.insert(costs[j]);
                j--;
            }
