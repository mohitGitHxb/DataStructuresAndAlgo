...
Output: 1
    ...
    @ Approach
    & calculate sum of customers satisfied before using secret technique
    & consider all windows of size exactly `minutes` and find the max sum
     */
public:
    const int mod = 1e9 + 7;
    Solution()
    ...
    int maxSatisfied(vi &customers, vi &grumpy, int minutes)
    {
        int n = customers.size();
        int ans = 0, additional = 0;
        forn(i, n)
        {
            if (!grumpy[i])
            {
        ...
        while (end < n)
        {
            ...
            if (end - start + 1 == minutes)
            {
                additional = max(additional, curr);
            }
            end++;
        }
        // deb(additional);
