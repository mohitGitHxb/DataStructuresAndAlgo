...
Since two subsequences were created, we return 2. Note that another optimal solution is to partition nums into the two subsequences [1] and [2,3].
    ...
    & This is true as even if some elements aren't lying between mx and mn they still could be combined with them because at the end it doesn't matter in which order the mx and mn are lying. all that matters is they are the respective maximum and minimum of the subsequence 
    Hint 3
    ^Sort the array.
     */
public:
    ...
    int partitionArray(vector<int> &nums, int k)
    {
        ...
        int subsequence = 1;
        int curr = nums.front();
        while (upper_bound(all(nums), curr + k) != nums.end())
        {
            curr = *upper_bound(all(nums), curr + k);
            subsequence++;
        }
        return subsequence;
    }
