...
@ BRUTE FORCE:
% iterate over all possible triplets and check if they satisfy the condition O(N^3)
@ Optimized approach
& we can notice that a^a = 0 using this property and the fact that a1^a2^a3 = b1^b2^b3
& assume array as [a1,a2,a3,b1,b2,b3]
& the total amount of triplets would be size of subarray - 1;
...
public:
    ...
    int countTriplets(vector<int> &arr)
    {
        ...
        int n = arr.size();
        unordered_map<int, pair<int, int>> prefixMap;
        prefixMap[0] = {0, 1}; // sum of all index , count
        int prefix = 0, result = 0;
        for (int i = 0; i < n; i++)
        {
            prefix ^= arr[i];
            if (prefixMap.count(prefix))
                // i-j1 + i-j2 + i-j3.. ==> i*count-(sum of j's)
                result += i * prefixMap[prefix].second - prefixMap[prefix].first;
            prefixMap[prefix].first += i + 1;
            prefixMap[prefix].second++;
        }
        return result;
