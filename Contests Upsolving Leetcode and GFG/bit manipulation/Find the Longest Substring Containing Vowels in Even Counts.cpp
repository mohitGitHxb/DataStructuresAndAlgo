...
@ Observations:
& -> since we need vowels even number of times we can use bit manipulation
& -> 1^1 = 0, so any time our bitmask becomes 0 we can immediately conclude that all are even
& -> Now using a common technique of prefix sum + hashing we can know the occurrence of each of these masks
&-> simply find the occurrence and return the length of the longest substring

%- Initially came up with this but though of using map<vector<int>,int>> mp where vector<int> would be of size 5
%- But bitmask makes it way too simple
     */
public:
    Solution()
    {
    ...
    int findTheLongestSubstring(string &s)
    {
        ll ans = 0;
        ll n = s.size();
        unordered_map<unsigned int, int, custom_hash> maskMap;
        unsigned int mask = 0;
        forsn(i, 0, n)
        {
            if (isVowel(s[i]))
            ...
            if (mask == 0)
            { // means all are even because xor is zero
                ans = max<ll>(ans, i + 1);
            }
            else if (!maskMap.count(mask))
            {
                maskMap[mask] = i;
            }
            else
            {
                ans = max<ll>(ans, i - maskMap[mask]);
            }
        }
        return ans;
    }
