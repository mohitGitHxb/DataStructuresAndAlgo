 ...
 * Observations: 1) Bitwise OR is non reversible means if we have or of 1|2|5|10 we can't simply get or of 2|5|10 by removing one like currOr &= ~(nums[0]);
 * 2) That means we need to find a way to accommodate that.
 & Now we create a vector of size 32 and store the count of set bits in each index. This vector will be used to find the shortest subarray.
 & Two functions addOr and removeOr will be used to add and remove the count of set bits in each index.
 & convertToNumber will be used to convert the vector to number.
 &) Now we can simply use the sliding window template and we can update the OR value of current window by simply accessing the bitCount vector.
...
public:
    ...
    // void printOr(int start, int end, vi &nums)
    // {
    //     int ans = 0;
    //     forsn(i, start, end + 1)
    //     {
    ...
    int minimumSubarrayLength(vector<int> &nums, int k)
    {
        ...
        while (end < sz(nums))
        {
            ...
            while (currOr >= k && start <= end)
            {
                ...
                // customPrint(bitCount);
                removeOr(nums[start]);
                // deb(currOr);
                // customPrint(bitCount);
                // deb(convertToNumber());
                currOr = convertToNumber();
                start++;
                // printOr(start, end, nums);
            }
            end++;
        }
...
{
    Solution obj;
    vector<int> nums = {1, 2, 12, 50, 96, 1};
    obj.minimumSubarrayLength(nums, 93);
    return 0;
}
