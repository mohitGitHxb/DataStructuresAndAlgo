...
public:
    ...
    % we know the properties of XOR
    * it is a odd ones detector and a^b^b = a
    % if the number of values that can be increased using xor is even then we can always increase it WHY?
    & since it is given that given graph is actually a tree hence through observation we can conclude that propagation of xor value to any node is possible means
    & IT doesn't have to be directly connected
    % but what if the count of values that can be increased using xor is odd?
    & then we have two choices since we will have to sacrifice one value it can be done in two ways
    ? 1) increase the value that can be increased using xor and find the minimum element whose value is decreasing on being xored
    ? 2) leave the value as it is

     */
    ll maximumValueSum(vi &nums, int k, vvi &edges)
    {
        ll count = count_if(nums.begin(), nums.end(), [k](int x)
                            { return (x ^ k) > x; });
        ll sum = 0;
        if (count & 1)
        {
            int sacrificedValue = INT_MAX;
            bool flag = false;
            forv(i, nums)
            {
                if ((i ^ k) > i)
                {
                    sum += i ^ k;
                    sacrificedValue = min(sacrificedValue, (i ^ k) - i);
                    flag = true;
                }
                else
                {
                    sum += i;
                    sacrificedValue = min(sacrificedValue, i - (i ^ k));
                }
            }
            if (flag)
            {
