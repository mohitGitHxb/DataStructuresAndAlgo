...
public:
    int minSwaps(const string &s)
    {
        ...
        // we will simply observe the pattern like ]]][[[ because balanced parenthesis will be gone.
        // we can observe the pattern for different number of opening and closing brackets (opening_count == closing_count)
        // at the end we can conclude the result with the below formulae/
        return (count(all(stk), '[') + 1) / 2;
    }
};
