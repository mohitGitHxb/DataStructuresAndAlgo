...
public:
    ...
    /*
    @ Intuiton part or observation
    & Breaking down the problem in two parts
    &1) since we need to find the subarrays whose first and last element is same and also equal to the maximum element of the subarray we can divide this problem in two segments

    % To find the subarrays ending and starting with same elemenet we can maintain a map like data structure to store the number of occurences of the element in previous side and just by looking at the size of the map we can determine the subarray count

    % Since we also want to have the ending and starting element equal to the maximum element we need to make sure that no element greater than current ith element exists between any two indices

    & For eg. _ _ 999999 _3 _ _ 4 3 _ _ 3 _ 3
    here we  can't form array with first 3 as there is a 4 that is after it

     */
    ll numberOfSubarrays(vi &nums)
    {
        stack<int> stk;
        vi previousGreator(sz(nums), -1);
        forn(i, sz(nums))
        {
            ...
            if (!stk.empty())
            {
                previousGreator[i] = stk.top();
            }
            stk.push(i);
        }
        ll subarrays = 0;
        // customPrint(previousGreator);
        unordered_map<int, vi> occurrences;
        forn(i, sz(nums))
        {
            occurrences[nums[i]].pb(i);
            ...
            {
                int largestNearestElement = previousGreator[i];
                if (largestNearestElement == -1)
                {
                    subarrays += occurrences[nums[i]].size();
                }
                else
                {
                    auto it = upper_bound(all(occurrences[nums[i]]), largestNearestElement);
                    int idx = it - occurrences[nums[i]].begin();
                    int elementToSkip = occurrences[nums[i]].size() - idx;
                    subarrays += elementToSkip;
                }
            }
            // customPrint(occurrences[nums[i]]);
        }
        return subarrays;
    }
};
