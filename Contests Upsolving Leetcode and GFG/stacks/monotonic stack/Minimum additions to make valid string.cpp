...
Consider the situation like if we have 'a' then it must be followed by a 'b' and similarly 'b' is followed by 'c' and 'c' must be followed by 'c'.
Approach

consider a stack initially with the first element. Now fist character must always be 'a' so if it is not add 'a' and increment count by 1, similarly the string must end with 'c' if it is not there at it and increment count by 1. Now push the elemnt in a stack and if top element is 'a' it must be followed by 'b' if 'b' is not there in string then push 'b' and increment count by 1. Do similar stuff for 'b' and 'c'
