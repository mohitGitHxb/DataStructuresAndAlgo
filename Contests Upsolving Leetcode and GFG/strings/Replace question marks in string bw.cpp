    ...
    string minimizeStringValue(string s)
    {
        ...
        for (char &c : s)
        {
            if (c == '?')
            {
                ...
                minHeap.pop();
                temp.pb(p.S);
                minHeap.push({p.F + 1, p.S});
            }
        }
        /* 
        ! WRONG SUBMISSION if you leave the below code
        & Make sure to sort the string in lexicographical order
        & assume a situation where best answer is like this abc?de... as abcZde..A..Y
        & but ZAY and AYZ results the same cost hence we need to sort the string to make it lexicographical smallest.
         */
        sortall(temp);
        int i = 0;
        for (char &c : s)
