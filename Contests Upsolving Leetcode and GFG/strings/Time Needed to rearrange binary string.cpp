...
public:
    ...
    {
        int seconds = 0, zerosPassed = 0;
        for (char c : s)
        {
            if (c == '0')
            {
                zerosPassed++; // Increment the count of zeros passed
            }
            else if (zerosPassed > 0)
            {
                // If we encounter a '1' and have passed at least one '0',
                // it will take 'zerosPassed' seconds to move this '1' past all the zeros
                seconds = max(seconds + 1, zerosPassed);
            }
        }
        return seconds;
    }
    int secondsToRemoveOccurrences(string s)
    {
        ...
        rforn(i, sz(s) - 1)
        {
            ...
            else if (onesPasses > 0)
            {
                seconds = max(seconds + 1, onesPasses);
            }
        }
        return seconds;
    }
