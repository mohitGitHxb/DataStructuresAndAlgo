...
public:
    ...
    // This function puts a key-value pair into a cache with frequency counts.

    void put(int key, int value)
    {
        // Check if the cache has capacity
        if (!capacity)
            return;
        ...
        if (keyVal.find(key) != keyVal.end())
        {
            ...
            // Update the position of the key in the frequency list
            pos[key] = --freqList[keyVal[key].second].end();

            // If the minimum frequency list is empty, update the minimum frequency
            if (freqList[minFreq].empty())
                minFreq++;
        ...
        // Add the new key-value pair to the cache with a frequency of 1
        keyVal[key] = {value, 1};
